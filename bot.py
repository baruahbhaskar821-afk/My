...
#=============== DATA STORAGE ================
sudo_users = set()  # CHANGE: Empty set, will add owner later
muted_users = set()
spam_active = False
spam_target = None
spam_count = 0
...
#=============== LOAD DATA ================
def load_data():
    global sudo_users, custom_stickers, shayari_data, muted_users
    ...
    try:
        ...
        if os.path.exists("muted_users.json"):  # ADDED: Load muted users
...
def save_data():
    ...
    with open("muted_users.json", "w") as f:  # ADDED: Save muted users
...
async def mute_command(client, message: Message):
    ...
    # FIX: Can't mute owner or sudo users
    if target_user.id == OWNER_ID or target_user.id in sudo_users:
        await message.reply_text("❌ Cannot mute owner or sudo users!")
        return
...
async def add_sudo_command(client, message: Message):
    ...
    if user_id:
        # FIX: Can't add owner as sudo (he's already supreme)
        if user_id == OWNER_ID:
            await message.reply_text("❌ Owner is already supreme!")
            return
...
#=============== MESSAGE HANDLER (UPDATED - Deletes both text and stickers) ================
...
async def message_handler(client, message: Message):
    ...
    # SUDO PROTECTION - FIXED: Sudo users ke messages delete nahi honge
    if user_id in sudo_users:
        return

    # DELETE MUTED USER MESSAGES AND STICKERS
    if user_id in muted_users:
        try:
            await message.delete()  # This deletes both text messages AND stickers
        except:
            pass
...
def main():
    ...
    # FIX: Ensure owner is in sudo users
    ...
    print(f"🔇 Muted Users: {len(muted_users)}")
